...
def ler_dados_inseridos():
    ...
    if len(_nome) == 0 or len(_sobrenome) == 0 or len(email) == 0 or len(linguagens_lidas) == 0 or len(experiencia) == 0:
        messagebox.showwarning(title="Opa...", message="Algum campo de entrada está em branco. "
                                                       " Por favor, retorne e verifique.")
    else:
        try:
            with open("cadastros.json", "r") as dados:
                dados_lidos = json.load(dados)
        except FileNotFoundError:
            with open("cadastros.json", "w") as dados:
                json.dump(modelo, dados, indent=4)
        else:
            dados_lidos.update(modelo)
            with open("cadastros.json", "w") as dados:
                json.dump(dados_lidos, dados, indent=4)
